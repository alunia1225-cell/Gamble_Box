...
function hl(choice){
 ...
 // The outcome is committed the instant HIGH/LOW is pressed.
 ...
 const tick=now=>{
   ...
   // Start deep in LOW territory. Random shocks can send the line sharply upward
   // before it eventually settles at the preselected final side.
   ...
   if(p>0.15&&p<0.88){
     ...
     if(shockWindow!==lastShock){
       ...
       if(chance<0.38){
         shock=(Math.random()<.5?-1:1)*(36+Math.random()*65);
       }
     }
   }

   // If HIGH is the locked result, make a late breakout possible.
   // If LOW is locked, create occasional false breakouts before the drop.
   if(finalHigh && p>0.42 && p<0.75 && Math.random()<0.04) shock+=-(35+Math.random()*55);
   if(!finalHigh && p>0.42 && p<0.75 && Math.random()<0.04) shock+=(35+Math.random()*55);

   // During the final 18%, converge decisively to the locked outcome.
   const finalBlend=p>0.82?(p-0.82)/0.18:0;
   let y=lastY+basePull+wave+shock;
   const blended=targetY*finalBlend+y*(1-finalBlend);
   y=Math.max(20,Math.min(228,blended));
   ...
   if(p>=1){
     HL_BUSY=false;
     chart.classList.remove("hl-wild");
...
function heCpu(){
 ...
 setTimeout(()=>{if(gbAlive(H.token))heNext()},1800+Math.floor(Math.random()*1500));
}
function heNext(){
...
function heStreet(){
 H.street++;H.currentBet=0;H.raiseCount=0;H.lastRaiseSize=100;H.streetAggro=0;H.players.forEach(p=>p.bet=0);
